...
class GPTJEPASidecar(base.GPT):
    ...
    def _chunk_reset_mask(self, input_ids: mx.array) -> mx.array | None:
        if not self.sidecar_reset_on_bos:
            return None
        if input_ids.shape[1] % self.sidecar_chunk_size != 0:
            raise ValueError(
                f"TRAIN_SEQ_LEN={input_ids.shape[1]} must be divisible by SIDECAR_CHUNK_SIZE={self.sidecar_chunk_size}"
            )
        token_mask = input_ids == self.sidecar_reset_token_id
        return mx.any(
            token_mask.reshape(
                token_mask.shape[0],
                token_mask.shape[1] // self.sidecar_chunk_size,
                self.sidecar_chunk_size,
            ),
            axis=2,
        )
    ...
    def sidecar_states_from_hidden(
        ...
        for idx in range(chunks):
            if reset_mask is not None:
                chunk_reset = mx.expand_dims(reset_mask[:, idx].astype(base.COMPUTE_DTYPE), axis=-1)
                state = chunk_reset * zero_state + (1.0 - chunk_reset) * state
    ...
    def sidecar_token_condition(self, side_states: mx.array, token_len: int, reset_mask: mx.array | None = None) -> mx.array:
        ...
        side_read_input = mx.stop_gradient(side_states)
        if reset_mask is not None:
            side_read_input = (1.0 - mx.expand_dims(reset_mask.astype(base.COMPUTE_DTYPE), axis=-1)) * side_read_input
    ...
    def forward_hidden_with_aux(
        ...
        sidecar_reset_mask = self._chunk_reset_mask(input_ids)
        ...
        side_tokens = self.sidecar_token_condition(side_states, int(input_ids.shape[1]), reset_mask=sidecar_reset_mask)
...
def main() -> None:
    ...
    while True:
        ...
        for _ in range(args.grad_accum_steps):
            loss, grads, last_batch = auxlib.loss_and_grad_chunked(args, model, train_loader, step_loss_and_grad)
            accum = base.accumulate_flat_grads(accum, grads, grad_scale)
