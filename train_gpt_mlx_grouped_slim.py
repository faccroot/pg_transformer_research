#!/usr/bin/env python3
"""
Slim GPT trainer with grouped streaming when curriculum is disabled.

This is the promotion-oriented successor to the harmonic slim control. It keeps
the lightweight training loop and contiguous grouped-streaming schedule, but it
optimizes the model's built-in total training loss (`loss_terms()[0]`) rather
than raw CE only. That makes it suitable for the current plain/TurboQuant stack
and static auxiliaries such as early-exit.
...
It intentionally rejects dynamic features that depend on the full base trainer's
heavier control path.
"""
...
def validate_supported_features(args: Hyperparameters) -> None:
    ...
    if unsupported:
        ...
        raise ValueError(
            "train_gpt_mlx_grouped_slim.py only supports the plain/Turbo/current-size lane "
            f"plus static early-exit and curriculum gating. Unsupported: {joined}"
        )
...
def main() -> None:
    ...
    if num_streams != batch_seqs and not args.curriculum_enabled:
        raise ValueError(
            f"LONGCTX_NUM_STREAMS must match microbatch batch size for grouped slim trainer, got "
    ...
    if compiled:
        if uses_logic:
            ...
            compiled_loss_and_grad = (
                ...
                compiled_loss_and_grad_impl(x, y, operator_codes)
            )
        else:
            compiled_ce_loss_impl = mx.compile(
                lambda x, y: model.ce_loss(x, y),
            ...
            compiled_loss_and_grad = (
                lambda x, y, operator_codes=None, token_weights=None, teacher_logits=None, teacher_hidden=None,
                branch_plans=None, structural_branching_cfg_override=None, early_exit_aux_weight_override=None:
                compiled_loss_and_grad_impl(x, y)
            )
    else:
        if uses_logic:
    ...
    if args.curriculum_enabled and hasattr(train_loader, "summary"):
        ...
        log("grouped_slim:curriculum_enabled grouped_streaming_bypassed=1")
    if args.curriculum_enabled:
        log(
    ...
    def eval_val_for_model(
        ...
        *,
        ...
        return base.eval_val(
            args,
            eval_model,
            lambda x, y, operator_codes=None: eval_model.ce_loss(x, y, operator_codes),
            lambda x, operator_codes=None: eval_model.forward_logits(x, operator_codes),
            eval_tokens,
            base_bytes_lut,
    ...
    while True:
        ...
        if args.train_log_every > 0 and (step <= 10 or step % args.train_log_every == 0 or stop_after_step is not None):
            ...
            if args.curriculum_enabled and curriculum_phase is not None:
                extra += f" curriculum_phase:{curriculum_phase.name}"
            log(
    ...
    roundtrip_model = base.make_gpt(args, sp)
