#!/usr/bin/env python3
"""
GPT with a chunk-level causal structural sidecar.
...
separate CE-trained readout path. Unlike the earlier chunk-broadcast prototype,
chunk `c` is conditioned only on sidecar state from chunk `c-1`, so the scored
path stays causal inside each chunk.
"""
...
from sidecar_transition import SidecarTransitionResetController
import train_gpt_mlx as base
import train_gpt_mlx_jepa_aux as auxlib


def _split3(x: mx.array, size: int) -> tuple[mx.array, mx.array, mx.array]:
...
class Hyperparameters(base.Hyperparameters):
    ...
    sidecar_dynamic_budget: bool = bool(int(os.environ.get("SIDECAR_DYNAMIC_BUDGET", "0")))
    sidecar_budget_min_scale: float = float(os.environ.get("SIDECAR_BUDGET_MIN_SCALE", "0.50"))
    sidecar_budget_max_scale: float = float(os.environ.get("SIDECAR_BUDGET_MAX_SCALE", "1.50"))
    sidecar_operator_density_high: float = float(os.environ.get("SIDECAR_OPERATOR_DENSITY_HIGH", "0.02"))
    sidecar_operator_density_low: float = float(os.environ.get("SIDECAR_OPERATOR_DENSITY_LOW", "0.005"))
    sidecar_high_human_compressibility: float = float(
        os.environ.get("SIDECAR_HIGH_HUMAN_COMPRESSIBILITY", "0.50")
    )
    sidecar_low_human_compressibility: float = float(
        os.environ.get("SIDECAR_LOW_HUMAN_COMPRESSIBILITY", "0.33")
    )
    ...
    sidecar_transition_reset_enabled: bool = bool(int(os.environ.get("SIDECAR_TRANSITION_RESET_ENABLED", "0")))
    sidecar_transition_reset_cosine_threshold: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_COSINE_THRESHOLD", "0.80")
    )
    sidecar_transition_reset_cosine_sharpness: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_COSINE_SHARPNESS", "12.0")
    )
    sidecar_transition_reset_prior_weight: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_PRIOR_WEIGHT", "1.0")
    )
    sidecar_transition_reset_learned_weight: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_LEARNED_WEIGHT", "0.0")
    )
    sidecar_transition_reset_max_gate: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_MAX_GATE", "1.0")
    )
    sidecar_transition_reset_init_std: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_INIT_STD", "1e-3")
    )
    sidecar_transition_reset_sparsity_weight: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_SPARSITY_WEIGHT", "0.0")
    )
    sidecar_transition_reset_prior_align_weight: float = float(
        os.environ.get("SIDECAR_TRANSITION_RESET_PRIOR_ALIGN_WEIGHT", "0.0")
    )
...
class GRUSidecarCell(nn.Module):
    def __init__(self, state_dim: int):
        ...
        self.in_proj = base.CastedLinear(state_dim, state_dim * 3)
        self.h_proj = base.CastedLinear(state_dim, state_dim * 3)
        self.in_proj.weight = (
            mx.random.normal(self.in_proj.weight.shape, dtype=mx.float32) * (state_dim ** -0.5)
        ).astype(mx.float32)
        self.h_proj.weight = (
            mx.random.normal(self.h_proj.weight.shape, dtype=mx.float32) * (state_dim ** -0.5)
        ).astype(mx.float32)

    def __call__(self, x_t: mx.array, h_t: mx.array) -> mx.array:
        gates_x = self.in_proj(x_t).astype(mx.float32)
        gates_h = self.h_proj(h_t).astype(mx.float32)
        x_r, x_z, x_n = _split3(gates_x, self.state_dim)
        h_r, h_z, h_n = _split3(gates_h, self.state_dim)
        r = mx.sigmoid(x_r + h_r)
        z = mx.sigmoid(x_z + h_z)
        n = mx.tanh(x_n + r * h_n)
        out = (1.0 - z) * n + z * h_t.astype(mx.float32)
...
class GPTJEPASidecar(base.GPT):
    def __init__(
        ...
        *,
        ...
        sidecar_transition_reset_enabled: bool,
        sidecar_transition_reset_cosine_threshold: float,
        sidecar_transition_reset_cosine_sharpness: float,
        sidecar_transition_reset_prior_weight: float,
        sidecar_transition_reset_learned_weight: float,
        sidecar_transition_reset_max_gate: float,
        sidecar_transition_reset_init_std: float,
        sidecar_transition_reset_sparsity_weight: float,
        sidecar_transition_reset_prior_align_weight: float,
    ):
        super().__init__(
        ...
        if sidecar_chunk_size <= 1:
            raise ValueError(f"SIDECAR_CHUNK_SIZE must be > 1, got {sidecar_chunk_size}")
        ...
        self.sidecar_transition_reset = SidecarTransitionResetController(
            enabled=sidecar_transition_reset_enabled,
            cosine_threshold=sidecar_transition_reset_cosine_threshold,
            cosine_sharpness=sidecar_transition_reset_cosine_sharpness,
            prior_weight=sidecar_transition_reset_prior_weight,
            learned_weight=sidecar_transition_reset_learned_weight,
            max_gate=sidecar_transition_reset_max_gate,
        )
        self.sidecar_transition_reset_sparsity_weight = sidecar_transition_reset_sparsity_weight
        self.sidecar_transition_reset_prior_align_weight = sidecar_transition_reset_prior_align_weight
        self.sidecar_summary_query = (
        ...
        self.sidecar_cell = GRUSidecarCell(sidecar_state_dim)
        ...
        self.sidecar_transition_reset_proj = None
        if self.sidecar_transition_reset.learned_weight > 0.0:
            self.sidecar_transition_reset_proj = base.CastedLinear(sidecar_state_dim * 2, 1)
            self.sidecar_transition_reset_proj.weight = (
                mx.random.normal(self.sidecar_transition_reset_proj.weight.shape, dtype=mx.float32)
                * sidecar_transition_reset_init_std
            ).astype(mx.float32)
    ...
    def chunk_summary_from_hidden(self, hidden: mx.array) -> mx.array:
        if hidden.shape[1] % self.sidecar_chunk_size != 0:
            raise ValueError(
                f"TRAIN_SEQ_LEN={hidden.shape[1]} must be divisible by SIDECAR_CHUNK_SIZE={self.sidecar_chunk_size}"
            )
        chunks = hidden.reshape(
            hidden.shape[0],
            hidden.shape[1] // self.sidecar_chunk_size,
            self.sidecar_chunk_size,
            hidden.shape[2],
        )
        last = chunks[:, :, -1, :]
        ...
        if self.sidecar_summary_mode == "mean_last":
            return (0.5 * (mean.astype(mx.float32) + last.astype(mx.float32))).astype(hidden.dtype)
        return self._query_pool_chunks(chunks, self.sidecar_summary_query)

    def chunk_reduce_scores(self, scores: mx.array) -> mx.array:
        if scores.shape[1] % self.sidecar_chunk_size != 0:
            raise ValueError(
                f"TRAIN_SEQ_LEN={scores.shape[1]} must be divisible by SIDECAR_CHUNK_SIZE={self.sidecar_chunk_size}"
            )
        chunks = scores.reshape(
            scores.shape[0],
            scores.shape[1] // self.sidecar_chunk_size,
            self.sidecar_chunk_size,
        ).astype(mx.float32)
        if self.sidecar_polarity_pool == "last":
            return chunks[:, :, -1]
        if self.sidecar_polarity_pool == "mean":
            return mx.mean(chunks, axis=2)
        return mx.max(chunks, axis=2)
    ...
    def _transition_reset_prior(
        self,
        current_side_in: mx.array,
        previous_side_in: mx.array | None,
    ) -> tuple[mx.array, mx.array]:
        batch = int(current_side_in.shape[0])
        if not self.sidecar_transition_reset.enabled or previous_side_in is None:
            zero = mx.zeros((batch, 1), dtype=mx.float32)
            return zero, mx.ones((batch, 1), dtype=mx.float32)
        current_u = auxlib.l2_normalize(current_side_in.astype(base.COMPUTE_DTYPE)).astype(mx.float32)
        previous_u = auxlib.l2_normalize(previous_side_in.astype(base.COMPUTE_DTYPE)).astype(mx.float32)
        cosine = mx.sum(current_u * previous_u, axis=-1, keepdims=True)
        logits = (
            mx.array(self.sidecar_transition_reset.cosine_threshold, dtype=mx.float32) - cosine
        ) * mx.array(self.sidecar_transition_reset.cosine_sharpness, dtype=mx.float32)
        prior = mx.sigmoid(logits)
        return prior.astype(mx.float32), cosine.astype(mx.float32)

    def _transition_reset_gate(
        self,
        current_side_in: mx.array,
        state: mx.array,
        previous_side_in: mx.array | None,
        ...
        batch = int(current_side_in.shape[0])
        if not self.sidecar_transition_reset.enabled:
            zero = mx.zeros((batch, 1), dtype=mx.float32)
            one = mx.ones((batch, 1), dtype=mx.float32)
            return zero.astype(base.COMPUTE_DTYPE), zero, one, zero
        prior, cosine = self._transition_reset_prior(current_side_in, previous_side_in)
        learned = mx.zeros_like(prior)
        if self.sidecar_transition_reset_proj is not None:
            reset_features = mx.concatenate([current_side_in, state.astype(current_side_in.dtype)], axis=-1)
            # Zero-ish init should mean "no learned reset pressure", not 0.5 gate.
            learned_logits = self.sidecar_transition_reset_proj(reset_features).astype(mx.float32)
            learned = mx.maximum(mx.sigmoid(learned_logits) - mx.array(0.5, dtype=mx.float32), 0.0) * 2.0
        mixed = (
            mx.array(self.sidecar_transition_reset.prior_weight, dtype=mx.float32) * prior
            + mx.array(self.sidecar_transition_reset.learned_weight, dtype=mx.float32) * learned
        )
        gate = mx.minimum(
            mx.maximum(mixed, mx.array(0.0, dtype=mx.float32)),
            mx.array(1.0, dtype=mx.float32),
        ) * mx.array(self.sidecar_transition_reset.max_gate, dtype=mx.float32)
        return gate.astype(base.COMPUTE_DTYPE), prior, cosine, learned

    def sidecar_states_from_hidden(
        self,
        hidden: mx.array,
        chunk_polarity: mx.array | None = None,
        ...
        side_in = self.sidecar_in_proj(mx.stop_gradient(summary)).astype(base.COMPUTE_DTYPE)
        if self.sidecar_polarity_write and chunk_polarity is not None:
            side_in = side_in + (
                mx.expand_dims(chunk_polarity.astype(base.COMPUTE_DTYPE), axis=-1)
                * mx.tanh(self.sidecar_polarity_write_scale).astype(base.COMPUTE_DTYPE)[None, None, :]
            )
        batch = int(side_in.shape[0])
        chunks = int(side_in.shape[1])
        if initial_state is None:
            state = mx.zeros((batch, self.sidecar_state_dim), dtype=base.COMPUTE_DTYPE)
        else:
        ...
        reset_gates: list[mx.array] = []
        reset_priors: list[mx.array] = []
        reset_cosines: list[mx.array] = []
        reset_learned: list[mx.array] = []
        previous_side_in: mx.array | None = None
        for idx in range(chunks):
            reset_gate, reset_prior, reset_cosine, reset_learned_signal = self._transition_reset_gate(
                side_in[:, idx, :],
                state,
                previous_side_in,
            )
            state = (1.0 - reset_gate) * state
            state = self.sidecar_cell(side_in[:, idx, :], state)
            states.append(state)
            reset_gates.append(reset_gate)
            reset_priors.append(reset_prior.astype(base.COMPUTE_DTYPE))
            reset_cosines.append(reset_cosine.astype(base.COMPUTE_DTYPE))
            reset_learned.append(reset_learned_signal.astype(base.COMPUTE_DTYPE))
            previous_side_in = side_in[:, idx, :]
        side_states = mx.stack(states, axis=1) if states else side_in[:, :0, :]
        if reset_gates:
            side_reset_gates = mx.stack(reset_gates, axis=1)
            side_reset_priors = mx.stack(reset_priors, axis=1)
            side_summary_cosines = mx.stack(reset_cosines, axis=1)
            side_reset_learned = mx.stack(reset_learned, axis=1)
        else:
            side_reset_gates = mx.zeros((batch, 0, 1), dtype=base.COMPUTE_DTYPE)
            side_reset_priors = mx.zeros((batch, 0, 1), dtype=base.COMPUTE_DTYPE)
            side_summary_cosines = mx.ones((batch, 0, 1), dtype=base.COMPUTE_DTYPE)
            side_reset_learned = mx.zeros((batch, 0, 1), dtype=base.COMPUTE_DTYPE)
        return summary, side_states, state, side_reset_gates, side_reset_priors, side_summary_cosines, side_reset_learned
    ...
    def sidecar_token_condition(
        ...
        batch = int(side_states.shape[0])
        if initial_state is None:
            init = mx.zeros((batch, self.sidecar_state_dim), dtype=base.COMPUTE_DTYPE)
        else:
            init = initial_state.astype(base.COMPUTE_DTYPE)
            if init.ndim == 1:
                init = mx.broadcast_to(init[None, :], (batch, init.shape[0]))
            if int(init.shape[0]) != batch or int(init.shape[1]) != self.sidecar_state_dim:
                raise ValueError(
                    f"initial sidecar state shape {tuple(init.shape)} does not match "
                    f"(batch={batch}, state_dim={self.sidecar_state_dim})"
                )
        # Strict chunk-level causality: all tokens in chunk c read state from chunk c-1.
        side_read_input = mx.concatenate([init[:, None, :], side_states[:, :-1, :]], axis=1)
        ...
        side_tokens = mx.broadcast_to(
            side_read[:, :, None, :],
            (side_read.shape[0], side_read.shape[1], self.sidecar_chunk_size, side_read.shape[2]),
        ).reshape(side_read.shape[0], token_len, side_read.shape[2])
        return side_tokens
    ...
    def forward_hidden_with_aux(
        ...
        side_chunk_polarity = (
            self.chunk_reduce_scores(self.strip_registers(side_source_scores))
            if (self.sidecar_polarity_write and side_source_scores is not None)
            else None
        )
        (
            side_summary,
            ...
            side_reset_gates,
            side_reset_priors,
            side_summary_cosines,
            side_reset_learned,
        ) = self.sidecar_states_from_hidden(
            tap_hidden,
        ...
        return self.final_norm(self.strip_registers(x)), captured, {
            ...
            "sidecar_chunk_polarity": side_chunk_polarity,
            "sidecar_reset_gates": side_reset_gates,
            "sidecar_reset_priors": side_reset_priors,
            "sidecar_summary_cosines": side_summary_cosines,
            "sidecar_reset_learned": side_reset_learned,
    ...
    def sidecar_terms_from_states(
        ...
        if side_states.shape[1] <= self.sidecar_pred_offset:
            zero = mx.array(0.0, dtype=mx.float32)
            return zero, zero, zero
        u = auxlib.l2_normalize(side_states)
        ...
        spherical_loss = auxlib.spherical_uniformity(u)
        return pred_loss, sigreg_loss, spherical_loss

    def sidecar_reset_metrics_from_aux(self, aux: dict[str, mx.array | None]) -> tuple[mx.array, mx.array, mx.array, mx.array]:
        zero = mx.array(0.0, dtype=mx.float32)
        gates = aux.get("sidecar_reset_gates")
        priors = aux.get("sidecar_reset_priors")
        cosines = aux.get("sidecar_summary_cosines")
        learned = aux.get("sidecar_reset_learned")
        if gates is None or priors is None or cosines is None or learned is None or gates.shape[1] <= 0:
            return zero, zero, mx.array(1.0, dtype=mx.float32), zero
        return (
            mx.mean(gates.astype(mx.float32)),
            mx.mean(priors.astype(mx.float32)),
            mx.mean(cosines.astype(mx.float32)),
            mx.mean(learned.astype(mx.float32)),
        )

    def sidecar_reset_aux_losses_from_aux(self, aux: dict[str, mx.array | None]) -> tuple[mx.array, mx.array]:
        zero = mx.array(0.0, dtype=mx.float32)
        gates = aux.get("sidecar_reset_gates")
        priors = aux.get("sidecar_reset_priors")
        learned = aux.get("sidecar_reset_learned")
        if gates is None or priors is None or learned is None or gates.shape[1] <= 0:
            return zero, zero
        sparsity_loss = mx.mean(gates.astype(mx.float32))
        if self.sidecar_transition_reset_proj is None:
            prior_align_loss = zero
        else:
            prior_align_loss = mx.mean(
                mx.square(learned.astype(mx.float32) - mx.stop_gradient(priors.astype(mx.float32)))
            )
        return sparsity_loss, prior_align_loss
    ...
    def loss_terms(
        ...
        if aux_scale <= 0.0:
            ...
            reset_sparsity_loss, reset_prior_align_loss = zero, zero
        else:
            ...
            reset_sparsity_loss, reset_prior_align_loss = self.sidecar_reset_aux_losses_from_aux(aux)
        ...
        total = ce_loss + mx.array(aux_scale, dtype=mx.float32) * (
            ...
            + self.sidecar_transition_reset_sparsity_weight * reset_sparsity_loss
            + self.sidecar_transition_reset_prior_align_weight * reset_prior_align_loss
        ) + (
        ...
        return total, ce_loss, pred_loss, sigreg_loss, spherical_loss, reset_sparsity_loss, reset_prior_align_loss
...
def make_sidecar_gpt(args: Hyperparameters, sp: spm.SentencePieceProcessor | None = None) -> GPTJEPASidecar:
    return GPTJEPASidecar(
        **base.gpt_kwargs_from_args(args, sp),
        ...
        sidecar_transition_reset_enabled=args.sidecar_transition_reset_enabled,
        sidecar_transition_reset_cosine_threshold=args.sidecar_transition_reset_cosine_threshold,
        sidecar_transition_reset_cosine_sharpness=args.sidecar_transition_reset_cosine_sharpness,
        sidecar_transition_reset_prior_weight=args.sidecar_transition_reset_prior_weight,
        sidecar_transition_reset_learned_weight=args.sidecar_transition_reset_learned_weight,
        sidecar_transition_reset_max_gate=args.sidecar_transition_reset_max_gate,
        sidecar_transition_reset_init_std=args.sidecar_transition_reset_init_std,
        sidecar_transition_reset_sparsity_weight=args.sidecar_transition_reset_sparsity_weight,
        sidecar_transition_reset_prior_align_weight=args.sidecar_transition_reset_prior_align_weight,
    )
...
def sidecar_aux_budget_controller(args: Hyperparameters) -> SidecarAuxBudgetController:
    return SidecarAuxBudgetController(
        enabled=args.sidecar_dynamic_budget,
        min_scale=args.sidecar_budget_min_scale,
        max_scale=args.sidecar_budget_max_scale,
        operator_density_high=args.sidecar_operator_density_high,
        operator_density_low=args.sidecar_operator_density_low,
        high_human_compressibility=args.sidecar_high_human_compressibility,
        low_human_compressibility=args.sidecar_low_human_compressibility,
    )
...
def main() -> None:
    ...
    sidecar_budget_ctrl = sidecar_aux_budget_controller(args)
    ...
    log(
        f"sidecar:chunk_size:{args.sidecar_chunk_size} tap_layer:{model.sidecar_tap_layer_index} "
        ...
        f"summary_mode:{args.sidecar_summary_mode} pred_target_mode:{args.sidecar_pred_target_mode} "
        f"polarity_write:{int(args.sidecar_polarity_write)} polarity_pool:{args.sidecar_polarity_pool} "
        f"transition_reset:{int(args.sidecar_transition_reset_enabled)} "
        f"transition_reset_thr:{args.sidecar_transition_reset_cosine_threshold:.3f} "
        f"transition_reset_sharp:{args.sidecar_transition_reset_cosine_sharpness:.3f} "
        f"transition_reset_prior_w:{args.sidecar_transition_reset_prior_weight:.3f} "
        f"transition_reset_learned_w:{args.sidecar_transition_reset_learned_weight:.3f} "
        f"transition_reset_max:{args.sidecar_transition_reset_max_gate:.3f} "
        f"transition_reset_sparse_w:{args.sidecar_transition_reset_sparsity_weight:.4f} "
        f"transition_reset_align_w:{args.sidecar_transition_reset_prior_align_weight:.4f} "
        f"aux_start_frac:{args.sidecar_aux_start_frac} "
        f"aux_ramp_frac:{args.sidecar_aux_ramp_frac}"
    )
    if sidecar_budget_ctrl.enabled:
        log(
            f"sidecar_dynamic_budget:enabled min_scale:{sidecar_budget_ctrl.min_scale:.3f} "
            f"max_scale:{sidecar_budget_ctrl.max_scale:.3f} "
            f"op_density_high:{sidecar_budget_ctrl.operator_density_high:.4f} "
            f"op_density_low:{sidecar_budget_ctrl.operator_density_low:.4f} "
            f"high_human_compressibility:{sidecar_budget_ctrl.high_human_compressibility:.3f} "
            f"low_human_compressibility:{sidecar_budget_ctrl.low_human_compressibility:.3f}"
        )
    log(
    ...
    while True:
        ...
        if sidecar_budget_ctrl.enabled:
            batch_metrics = train_loader.last_batch_metrics() if hasattr(train_loader, "last_batch_metrics") else {}
            aux_scale = derive_sidecar_aux_scale(
                base_scale=aux_scale,
                phase_focus=curriculum_phase.focus if curriculum_phase is not None else None,
                controller=sidecar_budget_ctrl,
        ...
        if args.train_log_every > 0 and (step <= 10 or step % args.train_log_every == 0 or stop_after_step is not None):
            ...
            if last_batch is not None and (step <= 10 or step % max(args.train_log_every, 50) == 0):
                if aux_scale <= 0.0:
                    ...
                    mx.eval(ce_metric)
                    extra = " ce:{:.4f} sidecar_pred:0.0000 sidecar_sigreg:0.0000 sidecar_spherical:0.0000 sidecar_reset_sparse:0.0000 sidecar_reset_align:0.0000".format(
                    ...
                    reset_sparse_metric, reset_align_metric = model.sidecar_reset_aux_losses_from_aux(aux_metric)
                    ...
                    reset_gate_metric, reset_prior_metric, reset_cosine_metric, reset_learned_metric = model.sidecar_reset_metrics_from_aux(aux_metric)
                    mx.eval(
                        ce_metric,
                        pred_metric,
                        sig_metric,
                    ...
                    extra = (
                        ...
                        f" sidecar_reset_sparse:{float(reset_sparse_metric.item()):.4f}"
                        f" sidecar_reset_align:{float(reset_align_metric.item()):.4f}"
                        ...
                        f" sidecar_reset_gate:{float(reset_gate_metric.item()):.4f}"
                        f" sidecar_reset_prior:{float(reset_prior_metric.item()):.4f}"
                        f" sidecar_summary_cos:{float(reset_cosine_metric.item()):.4f}"
                        f" sidecar_reset_learned:{float(reset_learned_metric.item()):.4f}"
            ...
            log(
                ...
                f"sidecar_dynamic:{int(sidecar_budget_ctrl.enabled)} "
